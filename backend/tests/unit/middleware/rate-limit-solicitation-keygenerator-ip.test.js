...
describe("rateLimitMiddleware - solicitationRateLimiter keyGenerator com IP", () => {
  let solicitationKeyGenerator;
  ...
  it("deve executar keyGenerator do solicitationRateLimiter sem usuário autenticado (linha 184)", () => {
    if (!solicitationKeyGenerator) return;
    const req = {
      headers: { "x-forwarded-for": "192.168.1.30" },
      connection: { remoteAddress: "192.168.1.30" },
    };
    const key = solicitationKeyGenerator(req);
    expect(key).toBe("solicitation:192.168.1.30");
  });
});
